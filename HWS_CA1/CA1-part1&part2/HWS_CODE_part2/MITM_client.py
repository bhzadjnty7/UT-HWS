...
# Configuration
HOST = '127.0.0.1'
PORT = 65433  # Connect to MITM proxy instead of reader directly
...
def connect_to_reader():
    ...
    print("\033[94m RFID TAG CONNECTING THROUGH MITM PROXY \033[0m")
    print("\033[94m" + "="*70 + "\033[0m")
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            print(f"\033[94m[*] Connecting to MITM proxy at {HOST}:{PORT}...\033[0m")
            s.connect((HOST, PORT))
            print("\033[94m[+] Connected to MITM proxy\033[0m")
            logging.info(f"Connected to MITM proxy at {HOST}:{PORT}")
            
            # Step 3: Receive M1 and H1 from reader (via MITM)
            data = s.recv(1024).decode()
            M1, H1 = data.split(',')
            print("\033[94m[+] Received authentication challenge\033[0m")
            ...
            try:
                ...
                if H1 == H1_calculated:
                    ...
                    print("\033[94m[*] Sending response...\033[0m")
                    ...
                    try:
                        ...
                        print("\033[91m[-] No response received from reader\033[0m")
                        logging.error("No response received from reader")
                else:
                    print("\033[91m[-] Reader challenge verification failed! H1 mismatch\033[0m")
                    print("\033[91m[!] This indicates a possible MITM attack modifying M1\033[0m")
                    logging.error("Reader challenge verification failed: H1 mismatch")
                    logging.warning("Possible MITM attack detected")
            except ValueError:
                print("\033[91m[-] Invalid R1 value received\033[0m")
                logging.error("Invalid R1 value received")
    
    except ConnectionRefusedError:
        print("\033[91m[-] Connection refused. Make sure the MITM proxy is running.\033[0m")
        logging.error("Connection refused. MITM proxy not available.")
...
if __name__ == "__main__":
    ...
    if not os.path.exists('client_mitm.log'):
        open('client_mitm.log', 'w').close()
